...
- **Current scope:** local Hyprland desktop activity capture plus idle periods, and what was playing over the user D-Bus (MPRIS) polled on its own interval alongside it, stored locally in SQLite. `daytrace today` renders the desktop activity as an aggregated narrative of blocks by default, with the pre-aggregation report of one row per stored segment behind `today --raw`; media is printed as a daily section apart from the desktop blocks in either case, never summed into their time. Browser window titles are stored by name, using the same free-text redaction scan as every other title; private/incognito detection is best-effort until a browser extension provides a structured signal, and it covers desktop capture only: MPRIS carries no private-window signal, so media playing behind a private browser window is recorded by name regardless of whether that window was recognized. Do not add cloud sync, screenshots, clipboard capture, page-body capture, dashboard UI, AI narrative generation, multi-user design, or tracking beyond this desktop and its media without a present need and explicit decision.
...
- Record what held the attention **by name**: window titles, media track titles and artists, and the address of what was playing. A tool that cannot say what consumed the largest block of the day does not answer the question it exists for, and for an ordinary window the same facts already sit in the browser's own history in more detail. Private-window detection is best-effort and desktop-only: a window it fails to skip writes its title into the store, and MPRIS carries no private-window signal at all, so media playing behind a private browser window is recorded whether or not the title detector caught that window. Both residuals are accepted rather than unnoticed; the blacklists are how a user closes what a redaction rule cannot.
